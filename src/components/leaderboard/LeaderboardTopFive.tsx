...
/** Mock data for UI only — replace with API data later. */
...
function RunnerUpRow({ entry, delayIndex, isLast }: { entry: LeaderboardEntry; delayIndex: number; isLast: boolean }) {
  return (
    <motion.li
      {...stagger(delayIndex)}
      className={cn(
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate font-display text-sm font-semibold text-foreground">{entry.username}</p>
        <p className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground">Rank {entry.rank}</p>
      </div>
      <div className="flex shrink-0 items-center gap-1.5 text-primary">
        <TrendingUp className="h-3.5 w-3.5" aria-hidden />
        <span className="font-display text-sm font-bold tabular-nums">+{entry.profitPct.toFixed(2)}%</span>
      </div>
    </motion.li>
  );
}
...
function PodiumSlot({ entry, place, delayIndex }: PodiumSlotProps) {
  ...
  return (
    ...
    >
      {/* Single rounded shell: border follows full silhouette (no mid divider) */}
      <div
        className={cn(
          "relative flex h-full min-h-0 w-full flex-col rounded-2xl border-2 bg-card/40 backdrop-blur-sm transition-shadow",
          /* Gold crown sits above the box (-top-3); overflow-visible so it isn't clipped */
      ...
      >
        ...
        >
          ...
          <p className="w-full truncate px-0.5 font-display text-sm font-semibold text-foreground">{entry.username}</p>
        ...
        {/* Base: same outer border; soft vignette only — no horizontal rule */}
        <div
          className={cn(
...
/**
 * Top 5 leaderboard UI: podium for ranks 1–3 and a grey stacked list for 4–5.
 */
export function LeaderboardTopFive({ entries = MOCK_LEADERBOARD_TOP_FIVE, className }: LeaderboardTopFiveProps) {
  const sorted = [...entries].sort((a, b) => a.rank - b.rank).slice(0, 5);
  ...
  const runnersUp = sorted.filter((e) => e.rank >= 4);

  if (sorted.length === 0) return null;

  return (
    <div className={cn("space-y-8", className)}>
      {/* Arena: backdrop clipped to radius; outer overflow-visible so gold crown isn’t cut off */}
      <div className="relative overflow-visible rounded-2xl border border-border/40">
        <div className="pointer-events-none absolute inset-0 overflow-hidden rounded-2xl" aria-hidden>
          {/* Depth base — cool navy → near-black → violet undertone */}
          ...
          {/* Side accents — gold / blue (podium energy) */}
      ...
      {runnersUp.length > 0 && (
        <div className="space-y-4">
          ...
          >
            ...
            <span className="text-[11px] text-muted-foreground">One step from the podium · ranks 4 and 5</span>
          </motion.div>
